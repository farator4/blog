 ...
 * @since Jolene 1.1.0
 */
add_action( 'customize_register', 'jolene_customize_register_fonts' );
function jolene_customize_register_fonts( $wp_customize ) {

	$defaults = jolene_get_defaults();
	
	class Jolene_Customize_Reset_Control extends WP_Customize_Control {
		public $type = 'button';
		...
		public function render_content() {
			...
			<form>
				...
				<input type="button" onclick="jolene_refresh()" value="<?php echo esc_attr( $this->value() ); ?>" <?php $this->link(); ?> />
			</form>
			<?php
		}
	...
	$wp_customize->add_panel( 'fonts', array(
		'priority'       => 5,
		'title'          => __( 'Customize Fonts', 'jolene' ),
		'description'    => __( 'Google Fonts', 'jolene' ),
	) );
...
//Font for site
	$wp_customize->add_setting( 'body_font', array(
		...
		'sanitize_callback' => 'jolene_sanitize_font_choice'
	) );
	$wp_customize->add_control( 'body_font', array(
		'label'   => __( 'Body Font', 'jolene' ),
		'section' => 'fonts',
		'settings'   => 'body_font',
		'type'       => 'select',
		'priority'   => 2,
		'choices'	 => jolene_get_font_choices(),
	) );	
...
//Font for site
	...
	$wp_customize->add_control( 'font_size', array(
		'label'      => __( 'Body font size(px)', 'jolene' ),
		'section'    => 'fonts_size',
		'settings'   => 'font_size',
		'type'       => 'number',
		'input_attrs' => array(
...
//Font for headings
	...
	$wp_customize->add_control( 'heading_font_size', array(
		'label'      => __( 'Heading font size(px)', 'jolene' ),
		'section'    => 'fonts_size',
...
//Font for headings
	$wp_customize->add_setting( 'heading_weight', array(
		...
		'capability'     => 'edit_theme_options',
		'sanitize_callback' => 'jolene_sanitize_weight_choice'
	) );
	$wp_customize->add_control( 'heading_weight', array(
		...
		'type'       => 'select',
		'priority'   => 2,
		'choices'	 => jolene_get_weight_choices(),
	) );
	
//New section in customizer: Reset
	$wp_customize->add_section( 'reset', array(
		'title'          => __( 'Reset', 'jolene' ),
		'description'          => __( 'Reset font settings', 'jolene' ),
		'priority'       => $section_priority++,
		'panel'  => 'fonts',
	) );
	...
	$wp_customize->add_setting( 'reset_fonts', array(
		'type'			 => 'empty',
		'default'        => __( 'Reset', 'jolene' ),
		'sanitize_callback' => 'jolene_sanitize_url'
	) );
	
	$wp_customize->add_control( new Jolene_Customize_Reset_Control( $wp_customize, 'reset_fonts', array(
		'label'   => 'reset_fonts',
		'section' => 'reset',
 ...
 * @since Jolene 1.1.0
 */
function jolene_sanitize_font_choice( $value ) {
	if ( '0' == $value ) {
		return '0';
	} else if ( array_key_exists( $value, jolene_get_font_choices() ) ) {
		return $value;
	} else {
		return 'Open Sans';
	}
 ...
 * @since Jolene 1.1.0
 */
function jolene_get_font_choices() {
	$fonts = jolene_get_google_fonts();
	$out = array();
 ...
 * @since Jolene 1.1.0
 */
function jolene_get_weight_choices() {
	$out = array( 'lighter' => __( 'Lighter', 'jolene' ), 
				  'normal' => __( 'Normal', 'jolene' ), 
				  'bold' => __( 'Bold', 'jolene' ));
	return $out;
}
 ...
 * @since Jolene 1.1.0
 */
function jolene_sanitize_weight_choice( $value ) {
	if ( '0' == $value ) {
		return '0';
	} else if ( array_key_exists( $value, jolene_get_weight_choices() ) ) {
		return $value;
	} else {
		return 'Open Sans';
	}
...
if ( ! function_exists( 'jolene_get_google_fonts' ) ) :
/**
 ...
 * @since Jolene 1.1.0
 */
function jolene_get_google_fonts() {
	return array(
		'0' => array(
			'label'    => __( 'None', 'jolene' ), 
		),		
		'ABeeZee' => array(
			'label'    => 'ABeeZee',
			'variants' => array(
 ...
 * @since jolene 1.1.0
*/
function jolene_hook_font_css() {
	$defaults = jolene_get_defaults();
	$body_font = get_theme_mod( 'body_font', $defaults['body_font'] );
	
	$heading_font = get_theme_mod( 'heading_font', $defaults['heading_font'] );
	if ( '0' == $heading_font )
		$heading_font = $body_font;
		
	$header_font = get_theme_mod( 'header_font', $defaults['header_font'] );
	if ( '0' == $header_font )
...
?>
	...
	<?php endif; ?>
		font-size: <?php echo esc_attr( get_theme_mod( 'font_size', $defaults['font_size'] ) ); ?>px;
	}
	...
	h5,
	h6 {
		font-weight: <?php echo esc_attr( get_theme_mod( 'heading_weight', $defaults['heading_weight'] ) ); ?>;
	...
	h1 {
		font-size:<?php echo esc_attr( get_theme_mod( 'heading_font_size', $defaults['heading_font_size'] ) ); ?>px;
	}
...
add_action( 'wp_head', 'jolene_hook_font_css' );
 ...
 * @since jolene 1.1.0
*/
add_action( 'customize_controls_print_scripts', 'jolene_print_customizer_js_colors' );
function jolene_print_customizer_js_colors( ) {
	$defaults = jolene_get_defaults();
	...
	jolene_print_reset( $fields, 'reset_fonts' );
}
 ...
 * @since jolene 1.1.0
*/
function jolene_print_reset( $fields, $bind ) {

?>
<script type="text/javascript">
	
	function jolene_set_val(name, newVal) {
	...
	function jolene_refresh() { 
	
	<?php 
		foreach( $fields as $id => $value ) {
			?>
			jolene_set_val( '<?php echo esc_js( $id ); ?>', '<?php echo esc_js( $value ); ?>' );
			<?php
		}	
	?>
	}
